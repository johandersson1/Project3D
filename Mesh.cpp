...
bool Mesh::LoadBlendMaterials(ID3D11Device* device, std::string name)
{
	std::ifstream reader;
	...
	while (std::getline(reader, line))
	{
		...
		if (text == "map_Kd")
		{
			std::string diffuse;
			reader >> diffuse;
			material.diffuseTextures[0] = Texture(device, "Models/" + name + "/" + diffuse);
		}
		...
		if (text == "map_Kd")
		{
			std::string diffuse;
			reader >> diffuse;
			material.diffuseTextures[1] = Texture(device, "Models/" + name + "/" + diffuse);
		}
